...
st.set_page_config(page_title="GP Calculator (v6 fix5: fees+region hardened)", layout="wide")
st.title("Gross Profit Calculator — v6 (fix5)")
st.caption("Hardened: packaging fees mapping, region mapping, and safe merges.")
...
def resolve_packaging_fee(row, fees_map: dict) -> float:
    # If row has explicit fee, honor it
    ...
    try:
        ...
        pass
    # Else use packaging_type default
...
def load_rules(xls: bytes):
    ...
    if "packaging_type" not in rules2.columns:
        # if missing, default to crate (safer than crash)
    ...
    # normalize fees table
    if not {"packaging_type","default_fee_per_unit"}.issubset({c.strip().lower() for c in fees.columns.map(str)}):
...
def compute_freight(df, df_store_region, df_region_freight):
    ...
    # populate from store mapping using dict maps (dedup)
...
with tab_calc:
    ...
    # fees map (with safe defaults + sidebar overrides)
    ...
    cost_input = st.data_editor(cost_input,
        ...
        use_container_width=True, key="cost_input_v6fix5")
