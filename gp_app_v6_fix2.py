...
st.set_page_config(page_title="GP Calculator (v6 fix2: safe merge on sku)", layout="wide")
st.title("Gross Profit Calculator — v6 (fix2)")
st.caption("Coerces Sales/Rules SKU to string (strip + upper) before merge to avoid dtype mismatch.")
...
def normalize_sales(df_sales, df_rules):
    ...
    rules["sku"] = rules["sku"].astype(str).str.strip().str.upper()
...
def compute_freight(df, df_store_region, df_region_freight):
    ...
    if df_store_region is not None:
        ...
        if region_col is not None:
            ...
            if store_name_col and "store_name" in df.columns:
...
with tab_calc:
    ...
    cost_input = st.data_editor(cost_input,
        ...
        use_container_width=True, key="cost_input_v6fix2")
